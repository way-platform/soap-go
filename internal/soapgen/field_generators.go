...
func generateStructFieldWithInlineTypesAndContextAndParentAndFieldRegistryInternal(
	...
	if element.Ref != "" {
		if referencedElement := ctx.resolveElementRef(element.Ref); referencedElement != nil {
			...
			// If the element name collides with a simpleType/complexType, use the Wrapper suffix
			// to match the declaration generated by shouldUseWrapperForElement.
			goType := ctx.resolveElementGoType(referencedElement.Name)
