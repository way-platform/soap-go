...
func TestClient_SOAPFaultWith500Status(t *testing.T) {
	...
	defer server.Close()
	// Create client with no retries to focus on SOAP functionality
	client, err := NewClient(WithEndpoint(server.URL), WithMaxRetries(0))
	if err != nil {
		t.Fatalf("Failed to create client: %v", err)
	}
	// Create request envelope
	reqEnv, _ := NewEnvelope(WithBody([]byte(`<request>Test</request>`)))
	// Make the call - should return unified error with both HTTP error and SOAP fault
	ctx := context.Background()
	respEnv, err := client.Call(ctx, "", reqEnv)
	if err == nil {
...
func TestClient_SOAPFaultWith400Status(t *testing.T) {
	...
	// Create request envelope
	reqEnv, _ := NewEnvelope(WithBody([]byte(`<request>Test</request>`)))
	// Make the call - should return error with SOAP fault
	ctx := context.Background()
	respEnv, err := client.Call(ctx, "", reqEnv)
	if err == nil {
	...
	// Verify it's a 400 status code with SOAP fault
	...
	if soapErr.Fault != nil && soapErr.Fault.FaultCode != "Client" {
		t.Errorf("Expected fault code 'Client', got: %s", soapErr.Fault.FaultCode)
	}
	// Should return nil envelope since error contains the envelope
	if respEnv != nil {
		t.Fatal("Expected nil response envelope when there's an error")
